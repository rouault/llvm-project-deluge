...
const pas_heap_config deluge_hard_heap_config = DELUGE_HARD_HEAP_CONFIG;
...
static void* allocate_page(pas_segregated_heap* heap, pas_physical_memory_transaction* transaction)
{
    ...
    result = pas_reservation_free_heap_try_allocate_with_alignment(
        ...
        "deluge_hard_page", pas_delegate_allocation);
    ...
    void* boundary, pas_page_kind kind, pas_lock_hold_mode heap_lock_hold_mode)
{
    ...
    result = pas_page_header_table_add(
        &deluge_hard_page_header_table,
        PAS_SMALL_PAGE_DEFAULT_SIZE,
        pas_bitfit_page_header_size(DELUGE_HARD_HEAP_CONFIG.small_bitfit_config),
        boundary);
    pas_heap_lock_unlock_conditionally(heap_lock_hold_mode);
    return result;
}

void deluge_hard_destroy_page_header(pas_page_base* page, pas_lock_hold_mode heap_lock_hold_mode)
{
    pas_heap_lock_lock_conditionally(heap_lock_hold_mode);
    pas_page_header_table_remove(&deluge_hard_page_header_table,
                                 PAS_SMALL_PAGE_DEFAULT_SIZE,
                                 page);
    pas_heap_lock_unlock_conditionally(heap_lock_hold_mode);
}
    ...
    size_t size, pas_alignment alignment, pas_large_heap* heap, const pas_heap_config* config)
{
    ...
    PAS_UNUSED_PARAM(heap);
    PAS_ASSERT(config == &deluge_hard_heap_config);
    ...
    allocation_result = pas_reservation_free_heap_try_allocate_with_alignment(
        ...
        "deluge_hard_large", pas_delegate_allocation);
...
void* deluge_hard_prepare_to_enumerate(pas_enumerator* enumerator)
{
    ...
    config = (const pas_heap_config*)pas_enumerator_read(
        enumerator, (void*)(uintptr_t)configs[pas_heap_config_kind_deluge_hard], sizeof(pas_heap_config));
    if (!config)
        return NULL;

    root_data = (deluge_hard_heap_config_root_data*)pas_enumerator_read(
        enumerator, config->root_data, sizeof(deluge_hard_heap_config_root_data));
    if (!root_data)
        return NULL;
...
bool deluge_hard_for_each_shared_page_directory(
