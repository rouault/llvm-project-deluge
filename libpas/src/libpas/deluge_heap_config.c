...
const pas_heap_config deluge_heap_config = DELUGE_HEAP_CONFIG;
...
void deluge_heap_config_activate(void)
{
#if PAS_OS(DARWIN)
    static const bool register_with_libmalloc = true;
#endif
    
    pas_designated_intrinsic_heap_initialize(&deluge_int_heap.segregated_heap,
                                             &deluge_heap_config);
